...
- A `while` block (`{"trigger", "actions"}`, requires `until`) is live only while armed: its trigger joins `Store.active_triggers`, so watched directories and repos follow the armed state. `fire_while_rules` runs after `fire_until_rules` for each event and in the interval pass, so a closing event wins over a while on the same tick; it skips rules with `pendingUntil`, never arms or disarms, and runs the while actions as executor phase `:while` (log kind `while`, no cooldown update). Interval whiles pace from `whileEpoch`, stamped on every attempt, falling back to `armedEpoch`.
