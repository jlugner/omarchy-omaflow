...
cat >"$rules_dir/office-branch.json" <<'EOF'
{
  ...
  "while": {
    ...
    "actions": [{"type": "notify", "message": "switched to {{branch}} from {{from}}"}]
  },
  "until": {
...
cat >"$rules_dir/pulse.json" <<'EOF'
{
  ...
  "while": {"trigger": {"type": "interval", "minutes": 5}, "actions": [{"type": "notify", "message": "pulse"}]},
...
run_env jq -e 'has("office-branch")' "$state/armed.json" >/dev/null
run_env jq -e '.kind == "while" and .ruleId == "office-branch" and .status == "ok" and .trigger == "while:git-branch-changed"' \
...
run_env "$plugin_dir/bin/omaflow-eval" tick >/dev/null
! grep -q 'omarchy-notification-send.*pulse$' "$calls"
set_armed '.pulse.armedEpoch -= 400'
run_env "$plugin_dir/bin/omaflow-eval" tick >/dev/null
[[ $(grep -c 'omarchy-notification-send.*pulse$' "$calls") == 1 ]]
run_env jq -e '.pulse.whileEpoch | type == "number"' "$state/armed.json" >/dev/null
run_env "$plugin_dir/bin/omaflow-eval" tick >/dev/null
[[ $(grep -c 'omarchy-notification-send.*pulse$' "$calls") == 1 ]]
set_armed '.pulse.whileEpoch -= 400'
run_env "$plugin_dir/bin/omaflow-eval" tick >/dev/null
[[ $(grep -c 'omarchy-notification-send.*pulse$' "$calls") == 2 ]]
run_env jq -e 'has("pulse")' "$state/armed.json" >/dev/null
set_armed '.pulse.armedEpoch -= 4000 | .pulse.whileEpoch -= 4000'
run_env "$plugin_dir/bin/omaflow-eval" tick >/dev/null
...
echo "$base,\"while\":{\"trigger\":{\"type\":\"custom\",\"name\":\"a\"},\"actions\":[{\"type\":\"notify\",\"message\":\"a\"}]}}" >"$test_root/no-until.json"
...
echo "$base,$until_block,\"while\":{\"trigger\":{\"type\":\"custom\",\"name\":\"a\"},\"actions\":[]}}" >"$test_root/empty.json"
expect_error "$test_root/empty.json" 'while.actions must be a non-empty array'
...
echo "$base,$until_block,\"while\":\"soon\"}" >"$test_root/string.json"
expect_error "$test_root/string.json" 'while must be an object'
echo "$base,$until_block,\"while\":{\"trigger\":{\"type\":\"custom\",\"name\":\"a\"},\"actions\":[{\"type\":\"notify\",\"message\":\"a\"}]}}" >"$test_root/good.json"
